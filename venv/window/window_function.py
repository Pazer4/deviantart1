    ...
    for i in file:
        ...
        for j in i:
            ...
            if j == ":":
                flag = 1
                flag2 = 1
        link += [stroka[1:]]#записывается всё,кроме первого элемента строки т.к. он :, лень делать по другому.
        stroka = ""
...
def func_picture_in_album(file_entry,grouping_combo,additional_load_combo,page_number):
    login, password = read_login_password()
    ...
    additional_load_flag=1
    if additional_load_combo != 0:
        additional_load_group=additional_load_combo.get()
        additional_load_flag=0
    ...
    for i in file:
        ...
        if file_split1[0] == grouping_name:
            ...
            for j in file_split2[0:-1]:
                file_split3 = j.split("::")
                if additional_load_flag == 0:
                    if file_split3[0] != additional_load_group:
                        continue
                    else:
                        additional_load_flag = 1
                        continue
    ...
    for i in new_dict.keys():
        ...
        link = new_dict[i]
        try:
            picture_in_album(browser, link, name_album, name_picture,page_number)
        except:
            print(f"Ошибка в загрузке на группе:{name_album} и картинке:{name_picture}")
